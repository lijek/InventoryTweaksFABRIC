...
public class SortingHandler extends Obfuscation {
    ...
    public SortingHandler(Minecraft mc, InvTweaksConfig config,
        ...
        for (int i = 0; i < size; i++) {
            ...
            ItemInstance stack = containerMgr.getItemStack(i);
            if (stack != null) {
        ...
        if (algorithm != ALGORITHM_DEFAULT) {
            ...
            if (algorithm == ALGORITHM_INVENTORY) {
                ...
                for (int i = size - 1; i >= 0; i--) {
                    ItemInstance from = containerMgr.getItemStack(i);
                    if (from != null) {

                        // Move armor parts
                        ItemBase fromItem = from.getType();
                        if (fromItem.getHasSubItems()) {
                             if (fromItem instanceof Armour) {
                                 Armour fromItemArmor = (Armour) fromItem;
                                 List<Slot> armorSlots = globalContainer.getSlots(ContainerSection.ARMOR);
                                 for (Slot slot : armorSlots) {
                                     if (slot.canInsert(from)
                                             && (!slot.hasItem() || fromItemArmor.field_2082 >
                                                     ((Armour) slot.getItem().getType()).field_2082)) {
                        ...
                        // Stackable objects are never damageable
                            ...
                            for (Integer lockPriority : lockPriorities) {
                                if (lockPriority > 0) {
                                    ItemInstance to = containerMgr.getItemStack(j);
                                    if (to != null && from.isEqualIgnoreFlags(to)) {
    ...
    private int putHoldItemDown() throws TimeoutException {
        ItemInstance holdStack = getHoldStack();
        if (holdStack != null) {
        ...
        if (lockPriorities[i] <= priority) {
            ...
            // Try to swap/merge
                ...
                if (!canBeSwappedOrMerged
                       && from.isEqualIgnoreFlags(to)
                       && getStackSize(to) < to.method_709() ) {
                    canBeSwappedOrMerged = true;
                }
                ...
                if (canBeSwappedOrMerged) {
                    ...
                    ItemInstance remains = containerMgr.getItemStack(i);
    ...
    private boolean isOrderedBefore(int i, int j) {
        ...
        ItemInstance iStack = containerMgr.getItemStack(i),
            ...
            if (keywordOrder[i] == keywordOrder[j]) {
                ...
                if (getItemID(iStack) == getItemID(jStack)) {
                    if (getStackSize(iStack) == getStackSize(jStack)) {
                        ...
                        return (damageDiff < 0 && !iStack.method_717()
                                || damageDiff > 0 && iStack.method_717());
                    } else {
    ...
    private int getItemOrder(ItemInstance item) {
