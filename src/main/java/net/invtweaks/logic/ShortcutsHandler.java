...
public class ShortcutsHandler extends Obfuscation {
    ...
    private ItemInstance fromStack;
    ...
    public void reset() {
        ...
        int upKeyCode = mc.options.forwardKey.key,
            downKeyCode = mc.options.backKey.key;
    ...
    public void handleShortcut(ContainerBase guiScreen) {
        ...
        int x = (ex * guiScreen.width) / mc.actualWidth;
        int y = guiScreen.height - (ey * guiScreen.height) / mc.actualHeight - 1;
            ...
            if (toIndex != -1) {
                ...
                case MOVE_ALL_ITEMS:
                {
                    for (Slot slot : container.getSlots(fromSection)) {
                        if (slot.hasItem() && areSameItemType(fromStack, slot.getItem())) {
    ...
    private void craftAll(boolean separateStacks, boolean drop) throws Exception {
        ...
        if (slot.hasItem()) {
            ...
            int idToCraft = getItemID(slot.getItem());
            do {
                container.move(fromSection, fromIndex, toSection, toIndex);
                toIndex = getNextIndex(separateStacks, drop);
                if (getHoldStack() != null) {
                    ...
                    && getItemID(slot.getItem()) == idToCraft
                    && toIndex != -1);
        }
    }
    ...
    private boolean haveControlsChanged() {
        return (!shortcutKeysStatus.containsKey(mc.options.forwardKey.key)
                || !shortcutKeysStatus.containsKey(mc.options.backKey.key));
    }
    ...
    private int getNextIndex(boolean emptySlotOnly, boolean drop) {
        ...
        if (!emptySlotOnly) {
            ...
            for (Slot slot : container.getSlots(toSection)) {
                if (slot.hasItem()) {
                    ItemInstance stack = slot.getItem();
                    if (stack.isEqualIgnoreFlags(fromStack)
    ...
    private Slot getSlotAtPosition(ContainerBase guiContainer, int i, int j) {
